        ...
        <div class="p-6 space-y-5">
            <div class="flex items-center gap-5 pb-5 border-b border-gray-100">
                <?php if ($a && !empty($a->foto)): ?>
                    <img src="<?= base_url('assets/images/uploads/admin/' . $a->foto) ?>?v=<?= isset($a->updated_at) ? strtotime($a->updated_at) : time() ?>" class="w-16 h-16 rounded-2xl object-cover border-2 border-hijau-100">
                <?php else: ?>
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-400"><?= $a && !empty($a->role_nama) ? $a->role_nama : 'Administrator' ?></div>
                </div>
            </div>
            ...
            <div>
                ...
                <?php if ($a && !empty($a->foto)): ?>
                    <img src="<?= base_url('assets/images/uploads/admin/' . $a->foto) ?>?v=<?= isset($a->updated_at) ? strtotime($a->updated_at) : time() ?>" class="w-14 h-14 rounded-xl object-cover mb-2 border border-gray-100">
                <?php endif; ?>
...
<script>
    const canEditAkun = <?= $can_edit ? 'true' : 'false' ?>;

    async function simpanAkun() {
        if (!canEditAkun) {
            showToast('Anda tidak punya izin edit akun.', 'error');
            return;
        }
